...
func (c *Cache) performRelativeNodeExpiry(ctx context.Context) (int, error) {
	...
	var removed int
	for _, node := range nodes {
		if node.Expiry().IsZero() || node.Expiry().Add(gracePeriod).After(latestExp) {
			continue
		}

		// if we get here, the node's expiration time was greater than `gracePeriod` earleir than our
		// `latestExp` value.  That means that we definitely should have seen an node expiry event by
		// now, and can safely assume that the backend's ability to expire entries is degraded.
